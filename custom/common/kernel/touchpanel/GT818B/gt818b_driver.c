...
#define TPD_KEYS_DIM            {{60,850,50,30},{180,850,50,30},{300,850,50,30}}
...
#if 0  //move down
        if ( (buf_status&0x30) != 0x20 )        //data not ready\A3\ACso return
        {
            TPD_DEBUG("[mtk-tpd] STATUS : %x\n", buffer[0]);
...
/*
	  ...
	  if ( (buf_status&0x30) != 0x20 )        //data not ready\A3\ACso return
        ...
        if ( finger_num > 5 )        //abnormal state \A3\ACso return
        {
